...
import { Tooltip, Icon, Button, Input, Popconfirm, Badge } from "antd";
...
function CancelledOrdersTable(props) {
    ...
    useEffect(() => {
        setPageNo(pageNo + 1);
        props.getCancelOrders(props.userId,pageNo,"Repair");
    }, [props.userId]);
    ...
    function handleRowData(item) {
      ...
      const handleInfiniteScroll = () => {
        setPageNo(pageNo + 1);
        props.getCancelOrders(props.userId,pageNo);
    ...
    return (
      ...
      <InfiniteScroll
      dataLength={props.cancelsOrders.length} // This is important to prevent reloading the same data
     next={handleInfiniteScroll}
     hasMore={hasMore}
     height={"62vh"}
    style={{width:"-webkit-fill-available"}}
    loader={props.fetchingCancelOrders?<div class="flex justify-center">Loading...</div>:null}
  initialLoad={true}
    >
                    {props.cancelsOrders.length > 0 ? (
                    props.cancelsOrders.map((item) => (
                                <div>
...
const mapStateToProps = ({ myorder, auth,}) => ({
    cancelsOrders:myorder.cancelsOrders,
    fetchingCancelOrders:myorder.fetchingCancelOrders,
...
export default connect(mapStateToProps, mapDispatchToProps)(CancelledOrdersTable);
