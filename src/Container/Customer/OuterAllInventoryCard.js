...
import {getAllInventorySupplierItemCs} from "./CustomerAction";
import { BundleLoader } from '../../Components/Placeholder';


function OuterAllInventoryCard (props) {
    const [page, setPage] = useState(0);
    const [hasMore, setHasMore] = useState(true);
  
    useEffect(() => {
     props.getAllInventorySupplierItemCs();
    }, []);
    ...
    if (props.fetchingInventorySupplierItemsC) {
      return <BundleLoader />;
    }
    ...
    return (
        ...
        pagination={false}
                        ...
                        >
                      {props.allInventorySupplierItemsC.map((item) => {
    ...
    const mapStateToProps = ({ customer,auth }) => ({
      allInventorySupplierItemsC: customer.allInventorySupplierItemsC,
      fetchingInventorySupplierItemsC:customer.fetchingInventorySupplierItemsC,
    ...
    const mapDispatchToProps = (dispatch) =>
      bindActionCreators(
        {
          getAllInventorySupplierItemCs,
    
        },
        dispatch
      );
    
    export default connect(mapStateToProps, mapDispatchToProps)(OuterAllInventoryCard);
