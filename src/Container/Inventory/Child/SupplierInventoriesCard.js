...
import {getSuplrInventorylist,LinkInventoryItem} from "../InventoryAction";
...
function SupplierInventoriesCard(props) {
    const [page, setPage] = useState(0);
  const [hasMore, setHasMore] = useState(true);

  useEffect(() => {
   props.getSuplrInventorylist(props.userId,page);
  }, []);
  ...
  const handleIncrement = (inventorySupplieId) => {
    const newUnit = (units[inventorySupplieId] || 0) + 1;
    setUnits((prevUnits) => ({
      ...prevUnits,
      [inventorySupplieId]: newUnit,
    }));
  ...
  const handleDecrement = (inventorySupplieId) => {
    if (units[inventorySupplieId] > 1) {
      const newUnit = units[inventorySupplieId] - 1;
      setUnits((prevUnits) => ({
        ...prevUnits,
        [inventorySupplieId]: newUnit,
      }));
  ...
  if (props.fetchingSuplrInventorylist) {
    return <BundleLoader />;
  }
  ...
  return (
    ...
    pagination={false}
                    ...
                    >
                  {props.suplrInventoryList.map((item) => {
...
const mapStateToProps = ({ inventory,auth }) => ({
    suplrInventoryList: inventory.suplrInventoryList,
  fetchingSuplrInventorylist:inventory.fetchingSuplrInventorylist,
...
export default connect(mapStateToProps, mapDispatchToProps)(SupplierInventoriesCard);
